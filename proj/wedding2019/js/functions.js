...
function loop() {
	...
	for (var i = 0; i < balls.length; i++) {
		//balls[i].edgeDetect();
		if (finish == false && change > 1000) {
			//定時時間到，回基底動畫
			finish = true;
		} else if (finish == false) {
			balls[i].color = 'rgb(' +(220+i/10) + ','+random(0, 255)+',200)';
			if (balls[i].x - 5 > XposAry1[i] || balls[i].x + 5 < XposAry1[i] || balls[i].y - 5 > YposAry1[i] || balls[i].y + 5 < YposAry1[i]) {
				...
				balls[i].color = 'rgb(' +(255-change/3) + ','+random(0, 255)+','+random(100, 180)+')';
				if (i == balls.length - 1) {
					//到定位計時
					change++;
				}
			...
			if (balls[i].x - 5 > XposAry2[i] || balls[i].x + 5 < XposAry2[i] || balls[i].y - 5 > YposAry2[i] || balls[i].y + 5 < YposAry2[i]) {
				...
				//基底動畫
				balls[i].color = 'rgb(' +(100+i/10) + ','+(180-i/10)+',250)';
				//XposAry2 = baseShap[0];
				//YposAry2 = baseShap[1];
